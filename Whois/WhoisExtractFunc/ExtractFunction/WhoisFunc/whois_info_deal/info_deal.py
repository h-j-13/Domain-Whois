...
from flag_manage import flag_manage

logger = static.logger()

# 进行数据处理
# param domain_punycode punycode格式的域名
# param func_name 处理函数名称
# param data 服务器返回数据
# return 域名whois信息字典
def get_result(domain_punycode, whois_addr, func_name, data):
    
    print domain_punycode, whois_addr, func_name
    #返回结果初始化
    ...
    if func_name == 'com_manage':
        domain_whois = function.com_manage(domain_punycode, whois_addr, data, domain_whois)
    else:
        try:
            domain_whois = eval('function.' + func_name + '(domain_whois, data)')
        except Exception, e:
            logger.error('domain: ' + domain_punycode + ' info_deal>error_info: ' + str(e))

    domain_whois = flag_manage(domain_whois) # 标记位确定
    # domain_whois = time_tran.time_tran(domain_whois) # 时间统一
    domain_whois['domain_status'] = domain_status.get_status_value(domain_whois['domain_status'], domain_whois['domain']) # 状态值确定
    domain_whois['hash_value'] = hash(domain_whois['details']) # 哈希值确定
    
    return domain_whois


# 含有二级服务器提取函数
def com_manage(domain_punycode, whois_addr, data, domain_whois):

    details = ""        # details用来保存多级查询时候，每次获得的whois信息
        
    domain_whois = sec_manage(data, domain_whois, "")

    # 已经提取到完整信息
    if flag_manage(domain_whois)['flag'] == 122:
    ...
    if xxx_bool(data):
        data = whois_connect.get_whois_recv.get_recv_info("=" + domain_punycode, whois_addr)
        domain_whois = sec_manage(data, domain_whois, "")
        details = details + str(data)

    sec_whois_server = get_sec_server(data, domain_punycode)
    if sec_whois_server:
        data = whois_connect.get_recv_info.get_recv_info(domain_punycode, sec_whois_server)
        details = details + str(data)
        domain_whois = sec_manage(data, domain_whois, sec_whois_server)
        domain_whois["sec_whois_server"] = sec_whois_server
    
    domain_whois["details"] = details.strip()
    return domain_whois
...
def get_sec_server(data, domain):
    ...
    domain = domain.upper()
    pos = data.find("Domain Name: %s" % domain)
    ...
    if pos == -1:
        ...
        if match:
            sec_whois_server = match[0].split(":")[1].strip()
...
def sec_manage(data, domain_whois, sec_whois_server):
    ...
    'whois.nayana.com', 'whois.allglobalnames.com', 'comnet-whois.humeia.com', 'whois.arcticnames.com',
    ...
    'whois.ilait.com', 'whois.badger.com', 'whois.binero.se', 'whois.dotearth.com', 'whois.maprilis.com.vn', 
    'nswhois.domainregistry.com', 'whois.worldbizdomains.com', 'whois.domainguardians.com', 'whois.boterosolutions.net', 
        ...
        for i in range(match_length):
            if match[i][0].find('Record created on') != -1:
                ...
                if len(infos[1].strip().split('  ')) == 2 and infos[1].strip().split('  ')[1].find('@') != -1:
                    ...
                    domain_whois['reg_name'] = infos[1].strip()
                if infos[len(infos) - 3].find('+') != -1 and infos[len(infos) - 3].strip().replace('+', '').replace('.', '').isdigit() == True:
                    domain_whois['reg_phone'] = infos[len(infos) - 3].strip()
...
def sec_oiinternet_manage(data, domain_whois):
    ...
    pattern = re.compile(r'(changed.*|registered_date.*|registerexpire_date.*|Registrant Contact Information(.*\n)+?\n)')
    match = pattern.findall(data)
    match_length = len(match)
    i = 0
    if match:
                    ...
                    if info.find('@') != -1:
                        for email in info.split(' '):
                            if email.find('@') != -1:
                                domain_whois['reg_email'] = email.strip()
                                domain_whois['reg_name'] = info.replace(email, '').strip()
                    elif info.find('+') != -1 and info.replace('+', '').replace('.', '').strip().replace(' ', '').isdigit():
                        domain_whois['reg_phone'] = info.strip()
...
def sec_sdsns_manage(data, domain_whois):
    ...
    pattern = re.compile(r'(Registrant Name.*|Registrant Phone.*|Registrant E-mail.*|Updated Date.*|Creation Date.*|Expiration Date.*)')
    match = pattern.findall(data)
    match_length = len(match)
    i = 0
    if match:
...
def sec_dotearth_manage(data, domain_whois):
    ...
    pattern = re.compile(r'(Record last updated on:.*|Record expires on.*|Record created on:.*|Registrant:(.*\n)+?.*Administrative Contact:)')
    match = pattern.findall(data)
    match_length = len(match)
    i = 0
    if match:
...
def sec_maprilis_manage(data, domain_whois):
    ...
    pattern = re.compile(r'(Database last updated on.*|Record expires on.*|Record created on.*|Registrant:(.*\n)+?.*Fax)')
    match = pattern.findall(data)
    match_length = len(match)
    i = 0
    if match:
...
def sec_worldbizdomains_manage(data, domain_whois):
    ...
    pattern = re.compile(r'(Record last updated:.*|Record expires.*|Record created:.*|Registrant Contact:(.*\n)+?.*Administrative Contact:)')
    match = pattern.findall(data)
    match_length = len(match)
    i = 0
    if match:
