...
import static
import get_func_name
import get_server_addr
import db_operation
import get_server_ip
import get_not_found_sign
...
time_server_function_file = time.mktime(time.strptime(server_function_file.readline().split('\n')[0].strip(),'%Y-%m-%d %H:%M:%S'))
...
time_server_not_found_file = time.mktime(time.strptime(server_not_found_file.readline().split('\n')[0].strip(),'%Y-%m-%d %H:%M:%S'))
